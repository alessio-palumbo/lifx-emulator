...
function draw(canvas,d,options={}){
 const s=d.Surface;const width=options.presentation?Math.max(1,canvas.parentElement.clientWidth):Math.max(200,canvas.parentElement.clientWidth),height=options.presentation?Math.max(1,canvas.parentElement.clientHeight):(d.Kind==='matrix'?160:100);const ratio=window.devicePixelRatio||1;if(canvas.width!==Math.round(width*ratio)||canvas.height!==height*ratio){canvas.width=Math.round(width*ratio);canvas.height=height*ratio;}canvas.style.width=width+'px';canvas.style.height=height+'px';const ctx=canvas.getContext('2d');ctx.setTransform(ratio,0,0,ratio,0,0);ctx.clearRect(0,0,width,height);
 if(d.Kind==='single_zone'){const radius=options.presentation?Math.min(width,height)*.3:32,centerY=options.presentation?height/2-radius*.12:44;ctx.fillStyle=cssColor(d.Colors[0],d.Power);ctx.shadowColor=ctx.fillStyle;ctx.shadowBlur=options.presentation?Math.max(28,radius*.8):28;ctx.beginPath();ctx.arc(width/2,centerY,radius,0,Math.PI*2);ctx.fill();ctx.shadowBlur=0;ctx.fillStyle='#68707f';ctx.fillRect(width/2-radius*.44,centerY+radius+radius*.05,radius*.88,Math.max(4,radius*.22));return;}
 const cells=[];
