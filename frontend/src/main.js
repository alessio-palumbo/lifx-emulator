...
<div id="error" role="alert"></div><p id="network-note" role="status"></p><main><section><div class="section-title"><div><h2>Your virtual lights</h2></div><div class="section-actions"><span id="count"></span><button id="presentation-all" type="button" class="presentation-launch" title="Open virtual room"><svg viewBox="0 0 24 24" aria-hidden="true"><rect x="3" y="4" width="18" height="14" rx="2"/><path d="M8 21h8M12 18v3"/><rect x="7" y="8" width="3" height="3" rx=".5"/><rect x="14" y="8" width="3" height="3" rx=".5"/></svg><span>Virtual room</span></button></div></div><div id="devices"></div></section><aside><section class="panel"><h2>Add a light</h2><form id="add"><label>Product<select id="product"></select></label><label>Label<input id="label" value="Virtual light" maxlength="32" required></label><div id="topology"></div><button class="primary">Create virtual light</button></form><p class="hint">Unique locally administered targets are generated automatically. Matrix dimensions describe physical packet layout; previews use the library surface mapping.</p></section><section class="panel activity"><h2>Recent traffic <button id="traffic-mode" type="button" class="traffic-mode" aria-pressed="false" title="Pause live traffic to inspect messages">Pause</button></h2><p class="hint" id="transport"></p><div id="activity" tabindex="0" aria-label="Recent LAN traffic"></div></section></aside></main>`;
const el=id=>document.getElementById(id);
const escape=s=>String(s).replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]));
const icons={
 disable:'<svg viewBox="0 0 24 24" aria-hidden="true"><circle cx="12" cy="12" r="8"/><path d="M8 12h8"/></svg>',
 enable:'<svg viewBox="0 0 24 24" aria-hidden="true"><circle cx="12" cy="12" r="8"/><path d="M8 12h8M12 8v8"/></svg>',
 remove:'<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M4 7h16M9 7V4h6v3M7 7l1 13h8l1-13M10 11v5M14 11v5"/></svg>'
};
const iconButton=(className,label,icon)=>`<button type="button" class="card-icon ${className}" aria-label="${label}" title="${label}">${icon}</button>`;
const zoneTooltip=document.createElement('div');zoneTooltip.id='zone-tooltip';zoneTooltip.hidden=true;zoneTooltip.setAttribute('role','tooltip');document.body.append(zoneTooltip);
const colorDetail=(label,color={})=>`${label} · H ${(Number(color.Hue)||0).toFixed(1)}° · S ${(Number(color.Saturation)||0).toFixed(1)}% · B ${(Number(color.Brightness)||0).toFixed(1)}% · K ${Number(color.Kelvin)||0}`;
let zoneTooltipTimer,tooltipTarget;
function positionZoneTooltip(){if(!tooltipTarget)return;zoneTooltip.style.left=`${Math.min(tooltipTarget.x+12,window.innerWidth-zoneTooltip.offsetWidth-8)}px`;zoneTooltip.style.top=`${Math.min(tooltipTarget.y+12,window.innerHeight-zoneTooltip.offsetHeight-8)}px`;}
function hideZoneTooltip(){clearTimeout(zoneTooltipTimer);zoneTooltipTimer=undefined;tooltipTarget=undefined;zoneTooltip.hidden=true;}
function bindZoneTooltip(canvas){
 if(canvas.dataset.tooltipBound)return;canvas.dataset.tooltipBound='true';
 canvas.addEventListener('pointermove',event=>{
  if(canvas.closest('#presentation')?.classList.contains('arranging')){hideZoneTooltip();return;}
  const rect=canvas.getBoundingClientRect(),scaleX=(canvas._drawWidth||rect.width)/Math.max(1,rect.width),scaleY=(canvas._drawHeight||rect.height)/Math.max(1,rect.height),x=(event.clientX-rect.left)*scaleX,y=(event.clientY-rect.top)*scaleY;
  const hit=(canvas._zoneHits||[]).find(item=>item.circle?Math.hypot(x-item.x,y-item.y)<=item.radius:x>=item.x&&x<=item.x+item.width&&y>=item.y&&y<=item.y+item.height);
  if(!hit){hideZoneTooltip();return;}
  if(tooltipTarget?.canvas===canvas&&tooltipTarget.text===hit.text){tooltipTarget.x=event.clientX;tooltipTarget.y=event.clientY;if(!zoneTooltip.hidden)positionZoneTooltip();return;}
  hideZoneTooltip();tooltipTarget={canvas,text:hit.text,x:event.clientX,y:event.clientY};
  zoneTooltipTimer=setTimeout(()=>{if(!tooltipTarget)return;zoneTooltip.textContent=tooltipTarget.text;zoneTooltip.hidden=false;positionZoneTooltip();zoneTooltipTimer=undefined;},350);
 });
 canvas.addEventListener('pointerleave',hideZoneTooltip);
}
...
function topology(){const p=products.find(p=>p.ID===Number(el('product').value));el('topology').innerHTML=p?.Multizone?'<label>Physical zones<input id="zones" type="number" min="1" max="255" value="16"></label>':p?.Matrix?'<div class="dimensions"><label>Send width<input id="width" type="number" min="1" max="255" value="8"></label><label>Height<input id="height" type="number" min="1" max="255" value="8"></label></div>'+(p.Chain?'<label>Chain length<input id="chains" type="number" min="1" max="16" value="5"></label>':'')+'<label>Orientation<select id="orientation"><option value="0">Right side up</option><option value="1">Upside down</option><option value="2">Face up</option><option value="3">Face down</option><option value="4">Left</option><option value="5">Right</option></select></label>':'';enhanceSelects(root);}
...
el('add').onsubmit=e=>{e.preventDefault();const n=(id,fallback=0)=>Number(el(id)?.value??fallback);action(()=>api().Add({Serial:'',Label:el('label').value,Product:n('product'),Enabled:true,Zones:n('zones'),Width:n('width'),Height:n('height'),Chains:n('chains',1),Orientations:Array(n('chains',1)).fill(n('orientation'))}));};
...
function render(){
 ...
 card.querySelector('.present').onclick=()=>presentation.openDevice(card.dataset.serial);
 ...
 card.querySelector('h3').textContent=d.Label;card.querySelector('.card-title p').textContent=d.Model;card.querySelector('.state').textContent=!d.Enabled?'Disabled':d.Power?'On':'Off';card.classList.toggle('disabled',!d.Enabled);card.querySelector('.metadata').textContent=`${d.Serial} · PID ${d.Product} · ${d.Kind.replaceAll('_',' ')}`;card.querySelector('.state').title=`Power level ${d.Power||0} (${Math.round((d.Power||0)/65535*100)}%)`;const enable=card.querySelector('.enable'),enableLabel=d.Enabled?'Disable':'Enable';enable.setAttribute('aria-label',enableLabel);enable.title=enableLabel;enable.innerHTML=d.Enabled?icons.disable:icons.enable;enable.classList.toggle('will-enable',!d.Enabled);draw(card.querySelector('canvas'),d);
 }
 ...
 el('presentation-all').disabled=!view.Devices.length;
 presentation.render(view.Devices);
 renderTraffic();
 enhanceSelects(root);
}
...
function draw(canvas,d,options={}){
 bindZoneTooltip(canvas);const hits=[];canvas._zoneHits=hits;
 const s=d.Surface;const width=options.presentation?Math.max(1,canvas.parentElement.clientWidth):Math.max(200,canvas.parentElement.clientWidth),height=options.presentation?Math.max(1,canvas.parentElement.clientHeight):(d.Kind==='matrix'?160:100);canvas._drawWidth=width;canvas._drawHeight=height;const ratio=window.devicePixelRatio||1;if(canvas.width!==Math.round(width*ratio)||canvas.height!==height*ratio){canvas.width=Math.round(width*ratio);canvas.height=height*ratio;}canvas.style.width=width+'px';canvas.style.height=height+'px';const ctx=canvas.getContext('2d');ctx.setTransform(ratio,0,0,ratio,0,0);ctx.clearRect(0,0,width,height);
 if(d.Kind==='single_zone'){const radius=options.presentation?Math.min(width,height)*.3:32,centerY=options.presentation?height/2-radius*.12:44;ctx.fillStyle=cssColor(d.Colors[0],d.Power);ctx.shadowColor=ctx.fillStyle;ctx.shadowBlur=options.presentation?Math.max(28,radius*.8):28;ctx.beginPath();ctx.arc(width/2,centerY,radius,0,Math.PI*2);ctx.fill();ctx.shadowBlur=0;ctx.fillStyle='#68707f';ctx.fillRect(width/2-radius*.44,centerY+radius+radius*.05,radius*.88,Math.max(4,radius*.22));hits.push({circle:true,x:width/2,y:centerY,radius,text:colorDetail('Color',d.Colors[0])});return;}
 const cells=[];
 ...
 const limit=options.presentation?Number.POSITIVE_INFINITY:(d.Kind==='matrix'?18:30);
 const step=Math.min((width-32-gaps)/s.Width,(height-24)/s.Height,limit);const offsetX=(width-step*s.Width-gaps)/2,offsetY=(height-step*s.Height)/2;
 for(const [x,y,chain=0]of cells){const index=y*s.Width+x,color=d.Colors[index],left=offsetX+x*step+chain*chainGap,top=offsetY+y*step,size=Math.max(1,step-2),label=d.Kind==='matrix'?`Cell ${x},${y}`:`Zone ${index}`;ctx.fillStyle=cssColor(color,d.Power);ctx.beginPath();ctx.roundRect(left,top,size,size,Math.min(3,step/4));ctx.fill();hits.push({x:left,y:top,width:size,height:size,text:colorDetail(label,color)});}
}
const presentation=createPresentation({root,draw,devices:()=>view.Devices,runtime:window.runtime});
el('presentation-all').onclick=()=>presentation.openAll();
