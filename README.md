...
Use **Preview** on a light to display it across the window, or open the **Virtual room** to place several live devices on a black stage. In the room, choose **Arrange** to drag devices or resize them from the lower-right corner. Positions and sizes are saved locally and scale with the window. **Full screen** uses the native application window when available; Escape returns to the main interface.
